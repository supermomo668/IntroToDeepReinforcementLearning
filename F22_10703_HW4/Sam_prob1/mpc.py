...
class MPC:
    ...
    def predict_next_state_model(self, states, actions):
        ...
        '''
        states is in form [particle1, particle2, ..., particle-n, ...] repeated popsize times
            For each of these particles, we sample the next state using a random model from the ensemble
            Return dim: [self.popsize * self.num_particles, self.state_dim]
        '''
        ...
        '''
        Networks take state concatenated with actions
        '''
        reps = int(states.shape[0]/actions.shape[0])
        actions_full = torch.repeat_interleave(torch.tensor(actions, dtype=torch.float), repeats=reps, dim=0)
        inputs = torch.cat((torch.tensor(states, dtype=torch.float), actions_full), dim=1)
        net_idx = np.random.choice(self.model.num_nets)
        mean, logvar = self.model.get_output(self.model.networks[net_idx](inputs))
        var_matrices = torch.stack([torch.diag(torch.exp(logvar[i])) for i in range(logvar.size(0)) ], 0)


        deltas = torch.distributions.multivariate_normal.MultivariateNormal(loc=mean, covariance_matrix=var_matrices).rsample()
        next_states = states + deltas.detach().cpu().numpy()
    ...
    def predict_next_state_gt(self, states, actions):
        ...
        for s,a in zip(states, actions):
            s_prime = self.env.get_nxt_state(s, a)
